...
pub mod chat_svc_server {
    ...
    #[async_trait]
    pub trait ChatSvc: std::marker::Send + std::marker::Sync + 'static {
        async fn talk(
            &self,
    ...
    impl<T> Clone for ChatSvcServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "llmchat.ChatSvc";
    impl<T> tonic::server::NamedService for ChatSvcServer<T> {
